...
class FindJam extends Component {
  ...
  sortJams = jams => {
    ...
    const filteredUpcomingJams = upcomingJams.filter(
      jam =>
        jam.members.findIndex(
          member => member === sessionStorage.getItem("userId")
        ) === -1
    );
    const openJams = filteredUpcomingJams.filter(
      jam =>
        jam.joinRequests.findIndex(
          joinRequest => joinRequest === sessionStorage.getItem("userId")
        ) === -1
    );
    const requestedJams = filteredUpcomingJams.filter(
      jam =>
        jam.joinRequests.findIndex(
          joinRequest => joinRequest === sessionStorage.getItem("userId")
        ) !== -1
          ...
          <div className="d-md-flex justify-content-around">
            <div className="new-jam-section col-12">
              ...
              <div className="row d-md-flex">
                {this.state.openJams && this.state.openJams.length ? (
                  this.state.openJams.map((jam, idx) => {
                    return (
                      <JamCard
                  ...
                  <p className="mx-auto no-info-yet-text">
                    There are no jams for you to join right now. Why don't you{" "}
                    <Link to="/createjam">create one</Link>?
                  </p>
                )}
              </div>
              <p className="text-center find-jam-section-title mt-3">
                Requested Jams
              </p>
              <hr />
              <div className="row d-md-flex">
                {this.state.requestedJams.length ? (
                  ...
                  <p className="mx-auto no-info-yet-text">
                    You have no pending jam requests.
                  </p>
                )}
              </div>
            </div>
