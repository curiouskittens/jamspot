...
class CreateJam extends Component {
    ...
    this.state.instruments.forEach(instrument => {
      if (!instrument.name) {
        ...
        sweetAlert(
          "error",
          "warning-text",
          "Please specify the number of players needed."
        );
        instrumentSelected = false;
        return;
      }
  ...
  handleFormSubmit = event => {
    ...
    const blankInstrumentsQuantity = this.state.instruments.filter(
      val => val.quantity === ""
    );
    ...
    if (!this.state.jamName) {
      ...
      !this.state.instruments[0].quantity
    ) {
      sweetAlert(
        "error",
        "warning-text",
        "Please add an instrument and the number of players needed."
      );
      ...
      sweetAlert(
        "error",
        "warning-text",
        "Please enter the number of players needed for each instrument."
      );
        ...
        <div className="create-jam-page-content container-fluid">
          <form>
            ...
            <div className="create-general-info-wrapper">
              <h4 className="text-center create-jam-section-title">
                General Info <i className="fas fa-info-circle" />
              </h4>
              <hr />
            ...
            <div className="create-instruments-wrapper">
              <h4 className="text-center create-jam-section-title">
                Instruments <i className="fas fa-drum" />
              </h4>
              <hr />
              {this.state.instruments.map((instrument, idx) => (
                <div
                  className="form-group create-jam-input-size"
                  key={`${idx}`}
                >
                  <InstrumentInput
                    instrument={instrument}
                    nameChangeHandler={this.handleInstrumentNameChange(idx)}
                    quantityChangeHandler={this.handleInstrumentQuantityChange(
                      idx
                    )}
            ...
            <div className="create-genres-wrapper">
              <h4 className="text-center create-jam-section-title">
                Genres <i className="fas fa-music" />
              </h4>
              <hr />
            ...
            <div className="text-center">
              <button
                className="btn btn-primary btn-lg create-jam-button"
          ...
          {this.state.jamCreated && (
            <Redirect to={`/jam/${this.state.createdJamId}`} />
          )}
        </div>
        <Footer />
      </div>
