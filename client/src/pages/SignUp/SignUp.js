...
class SignUp extends Component {
  ...
  handleInputChange = event => {
    ...
    if (name === "username" && value) {
      ...
      api.checkEmail(value).then(result => {
        if (result.data) {
  ...
  renderEmailStatus = () => {
    if (
      this.state.email &&
      (this.state.emailTaken || !this.validateEmail(this.state.email))
    ) {
      return (
        <small className="not-available-email">
          ✖ Sorry, this email has already been used or is invalid.
        </small>
      );
    } else if (
      this.state.email &&
      ...
      return <small className="available-email">✔ That email is valid.</small>;
    }
  };
  ...
  renderUsernameStatus = () => {
    if (this.state.usernameTaken) {
      return (
        <small className="not-available-username">
          ✖ Sorry, this username is not available.
        </small>
      );
    } else if (this.state.username && this.state.usernameTaken === false) {
      return (
        <small className="available-username">
          ✔ That username is up for grabs.
        </small>
      );
    }
  };
  ...
  renderPasswordStatus = () => {
    if (this.state.password && this.state.password.length < 6) {
      return (
        <small className="not-available-password">
          ✖ Password must be at least 6 characters.
        </small>
      );
    } else if (this.state.password) {
      return <small className="available-password">✔ Good to go.</small>;
    }
  };
        ...
        <div className="sign-up-wrapper container">
          <form className="sign-up-form">
            ...
            <div className="form-group">
              <label className="sign-up-label-text" htmlFor="name">
                Name:
              </label>
              <input
            ...
            <div className="form-group">
              <label className="sign-up-label-text" htmlFor="email">
                Email:
              </label>
              <input
                className="form-control"
            ...
            <div className="form-group">
              <label className="sign-up-label-text" htmlFor="username">
                Username:
              </label>
              <input
                className="form-control"
            ...
            <div className="form-group">
              <label className="sign-up-label-text" htmlFor="password">
                Password:
              </label>
              <input
                className="form-control"
            ...
            <button
              className="sign-up-btn btn btn-lg btn-primary"
              onClick={this.handleFormSubmit}
            >
              Sign Up
            </button>
          </form>
          <p className="sign-up-to-login-text">
